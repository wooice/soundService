...
public class UserSocialService implements com.sound.service.user.itf.UserSocialService {
  ...
  public List<User> recommandUsersByTags(List<String> tagLabels, Integer pageNum, Integer pageSize)
    ...
    Map<User, Integer> userFollowdSeq =
        SocialUtils.toSeqMap(SocialUtils.sortMapByValue(userFollowedNumMap, false));

    List<User> allResult =
        SocialUtils.combineLogicAndSocial(userTagSeq, userFollowdSeq,
            SocialUtils.DEFAULT_SOCIAL_POWER);

    return SocialUtils.sliceList(allResult, pageNum, pageSize);
  }

  @Override
  public List<Group> recommandGroupsByTags(List<String> tagLabels, Integer pageNum, Integer pageSize)
      throws UserException, SoundException {
    Map<User, Long> userTagNumMap = this.getUserTagNumMap(tagLabels);
    ...
    for (User user : userTagNumMap.keySet()) {
      ...
      for (Group group : user.getGroups()) {
        if (groupHitMap.containsKey(group)) {
          groupHitMap.put(group, groupHitMap.get(group) + userTagNumMap.get(user));
        } else {
    ...
    List<Group> allResult = SocialUtils.toSeqList(SocialUtils.sortMapByValue(groupHitMap, false));
    return SocialUtils.sliceList(allResult, pageNum, pageSize);
  }

  private Map<User, Long> getUserTagNumMap(List<String> tagLabels) throws SoundException {
  ...
  public static void main(String[] args) {
    ...
    List<Entry<String, Long>> result = SocialUtils.sortMapByValue(map, false);
