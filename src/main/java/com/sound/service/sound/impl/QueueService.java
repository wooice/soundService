...
public class QueueService implements com.sound.service.sound.itf.QueueService {
	...
	@Override
	public SoundLocal processSound(User user, String soundUrl, QueueNode node)
			throws SoundException, SoundAuthException, AudioProcessException {
		File waveFile = null;

		try {
			Extractor extractor = processFactory.getExtractor("wav");
			String soundInfoString = remoteStorageService.getSoundInfo(node
					.getFileName());
			...
			if (user.getUserRoles().contains(Constant.USER_ROLE)
				...
				if (totalDuration > user.getUserRoles().get(0)
						.getAllowedDuration() * 60) {
					throw new SoundException("TOTAL_LIMIT_ERROR");
				}
			}

			// Check weekly sum. User can't upload sounds more than
			// Constant.WEEKLY_ALLOWED_DURATION limitaion in recent one
			// week
			...
			try {
				...
				waveFile = new File(System.getProperty("java.io.tmpdir")
						+ node.getFileName() + ".png");
				waveFile.createNewFile();
				mapper.writeValue(waveFile, wave);
				this.remoteStorageService.uploadFile("wave", node.getFileName()
						+ ".png", waveFile.getAbsolutePath());
			} catch (AudioProcessException e) {
				throw e;
			} catch (Exception e) {
				throw new SoundException("PARSE_FAILED");
			}
			Sound sound = soundDAO.findOne("profile.remoteId",
					node.getFileName());
			if (null == sound) {
				sound = new Sound();
			...
			user.getUserSocial()
					.setSoundDuration(
							(long) (user.getUserSocial().getSoundDuration() + soundFormat
									.getDuration()));
			userDAO.save(user);
	...
	public void enqueue(QueueNode queueNode) {
		QueueNode node = queueNodeDAO.findOne("fileName",
				queueNode.getFileName());
		if (null == node) {
			node = queueNode;
	...
	public boolean checkUserRight(User user, SoundFormat soundFormat) {
		if (null == soundFormat.getAlbum_artist()
				&& null == soundFormat.getArtist()) {
			return true;
		}

		if (null != soundFormat.getAlbum_artist()
				&& Arrays.asList(soundFormat.getAlbum_artist().split("/"))
						.contains(user.getProfile().getAlias())) {
			return true;
		}
		...
		if (null != soundFormat.getComposer()
				&& Arrays.asList(soundFormat.getComposer().split("/"))
						.contains(user.getProfile().getAlias())) {
			return true;
		}
		...
		if (null != soundFormat.getPerformer()
				&& Arrays.asList(soundFormat.getPerformer().split("/"))
						.contains(user.getProfile().getAlias())) {
			return true;
		}
	...
	// public static void main(String[] args) throws AudioProcessException,
	// IOException
	// {
	// ObjectMapper mapper = new ObjectMapper();
	...
	// Wave wave = extractor.extractWaveByTotal(new BufferedInputStream(new
	// FileInputStream(new
	// File("C:\\server\\apache-tomcat-7.0.40\\temp\\test.wav"))), null);
	// File waveFile = new File(System.getProperty("java.io.tmpdir")+"aaa" +
	// ".png", "");
	// waveFile.createNewFile();
